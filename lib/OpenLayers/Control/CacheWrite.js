            ...
            try {
                ...
                if (this.imdb !== null && tile instanceof OpenLayers.Tile.Image &&
                    ...
                    try {
                        ...
                        if (canvasContext) {
                            var jpegRegex = /image(\/|%252F)jpeg/i;
                            var pngRegex = /image(\/|%252F)png/i;
                            var gifRegex = /image(\/|%252F)gif/i;
                            if (jpegRegex.test(url)) {
                              imageFormat = "image/jpeg";
                            }
                            else if (pngRegex.test(url)) {
                              imageFormat = "image/png";
                            }
                                ...
                                if (dataUrl != null && dataUrl != "") {
                                  tx.executeSql('insert into ' + tableName + ' values(?,?)',
                                      ...
                                      function(tx, err) // insert failure
                                      {
                                          ...
                                          if (err.code == 4) { // quota error
//                                            log("The database is full!");
                                          }
                                          else if (err.code == 1 && err.message.indexOf("constraint failed") > -1) {
...
//                                            log("Unknown database error " + err.code + ": " + err.message);
                                          }
                                      }
                                  );
                                }
                                else {
//                                  log("dataUrl is null or something: " + dataUrl);
                                }
                              },
 ...
 */
OpenLayers.Control.CacheWrite.clearCache = function(databaseName, tableName, doAlert) {
    var imdb = null;
    if (!!window.openDatabase) {
        try {
            imdb = openDatabase(databaseName, '1.0',
                    'OpenLayers Image Cache Database', 5*1024*1024);
        }
        catch (outererr) {
            log("probably failing to connect to database: " + JSON.stringify(outererr));
